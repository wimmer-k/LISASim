...
void SteppingAction::UserSteppingAction(const G4Step* aStep){
  ...
  */
  ...
  //G4cout<<"  "<<volume4->GetName()<<" "<<aStep->GetTotalEnergyDeposit()<<" deposit? "<<aStep->GetTrack()->GetDefinition()->GetParticleType()<<G4endl;
  ...
  if( aStep->GetTrack()->GetDefinition()->GetParticleType() == "nucleus" && aStep->GetPostStepPoint()->GetStepStatus() != fWorldBoundary ){
    ...
    if(G4StrUtil::contains(volume1->GetName(), "Diamond") && volume2->GetName() == "World" ){
      ...
      eventInfo->GetSimEvent()->SetOutGoingBeta(t,aStep->GetTrack()->GetStep()->GetPostStepPoint()->GetBeta());
