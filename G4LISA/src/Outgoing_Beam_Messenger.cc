...
{ 
  ...
  ExCmd->SetGuidance("Set the excitation energy for the beam-like reaction product (a level scheme file with the excited level is required).");
  ...
  TExCmd->SetGuidance("Set the excitation energy of the target-like reaction product (a level scheme file with the excited level is required).");
  ...
  ProbBefCmd = new G4UIcmdWithADouble("/BeamOut/ProbBef",this);
  ProbBefCmd->SetGuidance("Set the reaction probability in the window");
  ProbBefCmd->SetParameterName("choice",false);
  ProbBefCmd->AvailableForStates(G4State_PreInit,G4State_Idle);

  ProbInCmd = new G4UIcmdWithADouble("/BeamOut/ProbIn",this);
  ProbInCmd->SetGuidance("Set the reaction probability in the LISA setup");
  ProbInCmd->SetParameterName("choice",false);
  ProbInCmd->AvailableForStates(G4State_PreInit,G4State_Idle);

  ProbAftCmd = new G4UIcmdWithADouble("/BeamOut/ProbAft",this);
  ProbAftCmd->SetGuidance("Set the reaction probability in the LISA setup");
  ProbAftCmd->SetParameterName("choice",false);
  ProbAftCmd->AvailableForStates(G4State_PreInit,G4State_Idle);
...
{ 
  ...
  if( command == ProbBefCmd )
    { BeamOut->SetProb_Bef(ProbBefCmd->GetNewDoubleValue(newValue));}
  if( command == ProbInCmd )
    { BeamOut->SetProb_In(ProbBefCmd->GetNewDoubleValue(newValue));}
  if( command == ProbAftCmd )
    { BeamOut->SetProb_Aft(ProbBefCmd->GetNewDoubleValue(newValue));}
